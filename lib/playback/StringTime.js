...
(function (root, timeFactory) {  // root is usually `window`
	...
	*/
		...
		*/
			...
			var key = 'hasPeriod';
			if ( processed[ key ] && !stm.orDefault('skipSentenceDelay') ) {
				delayModKey = toMultiplyBy[ key ];
				delay *= stm.orDefault( delayModKey );
			}
			key = 'hasOtherPunc';
			if ( processed[ key ] && !processed['hasPeriod'] && !processed['isNumeric']) {
				delayModKey = toMultiplyBy[ key ];
				delay *= stm.orDefault( delayModKey );
			}
			key = 'isNumeric';
			if ( processed[ key ] && !processed['hasPeriod'] && !processed['hasOtherPunc']) {
				delayModKey = toMultiplyBy[ key ];
				delay *= stm.orDefault( delayModKey );
			}
			key = 'isShort';
			if ( processed[ key ] && !processed['hasPeriod'] && !processed['hasOtherPunc'] && !processed['isNumeric']) {
				delayModKey = toMultiplyBy[ key ];
				delay *= stm.orDefault( delayModKey );
			}
			key = 'isLong';
